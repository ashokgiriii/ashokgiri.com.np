...
const app = express();
const { PORT = 10000, CONTACT_RECEIVER, NODE_ENV } = process.env;
...
// Rate limiting — 3 contact messages per day per IP
...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "contact-api", emailConfigured: isEmailConfigured() });
});
...
app.post("/api/contact", contactLimiter, async (req, res, next) => {
  try {
    ...
    await sendContactEmail({ to: CONTACT_RECEIVER, replyTo: email, subject, name, message });
...
app.use((err, _req, res, _next) => {
  ...
  const safeClientError = err.expose && Number.isInteger(code) && code >= 400 && code < 600;
  res.status(safeClientError ? code : 500).json(
    safeClientError ? { error: err.message } : { error: "Internal Server Error" }
  );
});
